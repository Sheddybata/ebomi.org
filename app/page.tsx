...
import { ArrowRight, Clock, ChevronLeft, ChevronRight, MapPin, Play, Radio, Users, Globe, Handshake, Building2, Music, Pointer, Facebook, Youtube, ExternalLink, BookOpen, Calendar } from 'lucide-react'
...
export default function Home() {
  ...
  const featuredEvent = {
    title: 'Business and Leadership Summit 2026',
    theme: 'Foundation for Kingdom Wealth – Building From Bottom Up',
    hosts: 'Prophet Dr. Isa & Choice El-Buba',
    date: 'July 27th – 31st, 2026 (5 Days)',
    time: 'Morning Session: 8:00 AM Daily | Evening Session: 4:00 PM Daily',
    locations: [
      'EBOMI Temples & Towers, No. 1A Kashim Ibrahim Street, Jos, Plateau State, Nigeria.',
    ],
    registrationLink: 'https://docs.google.com/forms/d/e/1FAIpQLSdfk_QKod_sOQOtz2-P4lNA_oYYXB6dZZCFIl-qp4vOgdheSA/viewform?usp=header',
    images: ['/upcoming program/newprogram.jpeg'],
    startDate: new Date('2026-07-27T08:00:00'),
  }
  const featuredCongressHasEnded = false
  const [featuredImageIndex, setFeaturedImageIndex] = useState(0)

  useEffect(() => {
    if (featuredEvent.images.length <= 1) return
    const interval = setInterval(() => {
  ...
  return (
    <PullToRefresh onRefresh={handleRefresh}>
      ...
      {/* Upcoming Events Section - Business and Leadership Summit 2026 */}
            ...
            <div className="text-white space-y-4 sm:space-y-6 order-2 lg:order-2">
              ...
              <div className="flex items-start space-x-3 sm:space-x-4 bg-white/10 backdrop-blur-md rounded-xl p-4 sm:p-6 border border-white/20">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-white font-semibold text-xs sm:text-sm uppercase tracking-wider mb-1">Hosts</div>
                  <div className="text-white text-base sm:text-lg md:text-xl font-medium break-words">{featuredEvent.hosts}</div>
                </div>
              </div>
              ...
              <div className="flex items-start space-x-3 sm:space-x-4 bg-white/10 backdrop-blur-md rounded-xl p-4 sm:p-6 border border-white/20">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-white font-semibold text-xs sm:text-sm uppercase tracking-wider mb-1">Time</div>
                  <div className="text-white text-sm sm:text-base md:text-lg break-words">{featuredEvent.time}</div>
                </div>
              </div>
                  ...
                  <div className="text-white text-sm sm:text-base md:text-lg break-words space-y-2">
                    {featuredEvent.locations.map((venue, index) => (
                      <p key={index} className="leading-relaxed">
                        {featuredEvent.locations.length > 1 ? (
                          <>
